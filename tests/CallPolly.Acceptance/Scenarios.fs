        ...
        "upstreamC": {
            ...
            "policies": {
                "default": [
                    ...
                    { "rule": "LimitBy","maxParallel": 2, "maxQueue": 4, "tag": "clientId" },
                    { "rule": "LimitBy","maxParallel": 2, "maxQueue": 4, "tag": "userAgent" }
                ]
            }
        }
    }
...
type Sut(log : Serilog.ILogger, policy: CallPolly.Rules.Policy<_>) =
    ...
    member __.ApiMulti tags (a : Act) = run "ingres" "api-a" <| _apiC tags a
...
type Scenarios(output : Xunit.Abstractions.ITestOutputHelper) =
    ...
    let [<Fact>] ``Limit - Bulkhead functionality`` () = async {
        ...
        test <@ match counts with
                | [200,successCount; 503,rejectCount] -> successCount < 100 && rejectCount >= 900
                | x -> failwithf "%A" x @>
    ...
    let [<Fact(Skip="Fails atm")>] ``LimitBy - BulkheadMulti functionality`` () = async {
        ...
        let act i =
            match i % 3 with
            | 0 -> sut.ApiMulti ["clientId","A"]  (DelayMs 100)
            | 1 -> sut.ApiMulti ["clientIp","A"]  (DelayMs 100)
            | _ -> sut.ApiMulti ["userAgent","A"] (DelayMs 100)
            |> Async.Catch
        let! time, res = List.init 1000 act |> Async.Parallel |> Stopwatch.Time
        let counts = res |> Seq.countBy (|Status|) |> Seq.sortBy fst |> List.ofSeq
        test <@ match counts with
                | [200,successCount; 503,rejectCount] -> successCount < 20 && rejectCount > 980
                | x -> failwithf "%A" x @>
        test <@ between 0.3 2.5 (let t = time.Elapsed in t.TotalSeconds) @>
    }
